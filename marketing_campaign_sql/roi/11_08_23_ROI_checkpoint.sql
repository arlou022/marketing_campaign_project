...
SELECT
	...
	+ SUM(accepted_cmp_5) + SUM(response) ) * 11 AS total_revenue_all_cmp
FROM public.campaign
),
cost AS(
SELECT 	
...
SELECT 
    revenue.total_revenue_all_cmp,
	cost.total_cost_all_cmp,
	( revenue.total_revenue_all_cmp - cost.total_cost_all_cmp )
	AS total_marketing_return,
	ROUND( ( revenue.total_revenue_all_cmp - cost.total_cost_all_cmp )::numeric
